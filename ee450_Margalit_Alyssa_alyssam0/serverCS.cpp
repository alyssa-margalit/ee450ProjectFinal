...
void startUDP(){
    ...
    cout<<"The ServerCS is up and running using UDP on port 22555"<<endl;
}
...
vector<string> temp2;
void getClasses(){//read txt file, parse into string vectors
    ifstream in("cs.txt");
    string temp;
...
string courseQuery = "CS100";
string tempCourse;
// const char *responseM = "hello";
string responseM;
vector <string> parsed;
void parseCourseCode(){
    parsed.clear();//clear
    string temp = buffer;
...
void checkMatch(){//look for course and category
    parseCourseCode();
    cout<<"The ServerCS received a request from the Main Server about the "<<parsed.at(1)<<" of "<<courseQuery<<"."<<endl;
     ...
     for(size_t i = 0; i<classes.size();i++){
        ...
        if(index != string::npos){
            ...
            if(queryType=="CourseName"){//remove last character of line
           ...
           cout<<tx<<endl;
            ...
            // printf("\nThe course information has been found: The %s of %s is %s.",parsed.at(1).c_str(),courseQuery.c_str(),ts.c_str());
...
void MComms(){
    ...
    //     cout<<char_array[i];
    // }
   // cout<<"length of responseM is: "<<strlen(responseM)<<endl;
   // memset(&buffer, 0, sizeof(buffer));
   // sprintf(buffer,"%s",stuff.data());
...
int main() { 
  ...
  //  checkMatch();   
    ...
    while(1){
        // cout<<"\n\n";
        MComms();//send response whenever serverM provides a request
    }
